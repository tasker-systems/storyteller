...
impl DescriptorSet {
    ...
    pub fn load(base_dir: &Path) -> Result<Self, StorytellerError> {
        ...
        let goals = Self::load_required::<GoalsFile>(&desc_dir, "goals.json")?.goals;
...
mod tests {
    ...
    fn goals_descriptor_loads() {
        ...
        let set = DescriptorSet::load(&base).expect("descriptors should load");
        assert!(
            !set.goals.is_empty(),
        ...
        let goal = set
            .goals
            .iter()
            .find(|g| g.id == "protect_secret")
            .expect("protect_secret should exist");
    ...
    fn profiles_have_scene_goals() {
        ...
        let set = DescriptorSet::load(&base).expect("descriptors should load");
        let quiet_reunion = set
            .profiles
            .iter()
            .find(|p| p.id == "quiet_reunion")
    ...
    fn archetypes_have_pursuable_goals() {
        ...
        let stoic = set
            .archetypes
            .iter()
            .find(|a| a.id == "stoic_survivor")
            .expect("stoic_survivor should exist");
        assert!(stoic
            .pursuable_goals
            .contains(&"protect_secret".to_string()));
    }
    ...
    fn dynamics_have_enabled_and_blocked_goals() {
        ...
        let grief = set
            .dynamics
            .iter()
            .find(|d| d.id == "strangers_shared_grief")
            .expect("strangers_shared_grief should exist");
