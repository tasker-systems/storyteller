...
# Run LLM patch fills for ontological-posture type across all 30 genres.
...
echo "Starting ontological-posture LLM patch fill — $(date)"
...
for i in 1 2 3 4 5 6; do
    ...
    eval uv run narrative-data fill --tier llm-patch --type ontological-posture $(genre_flags "$GENRES")
    if [ "$i" -lt 6 ]; then
...
echo ""
echo "Ontological-posture patch fill complete — $(date)"
